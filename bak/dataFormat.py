...
def convert_data(*args):
    ...
    train_frame = train_frame.astype(int)
...
if __name__ == '__main__':
    ...
    # read data
    df = pd.read_table('{}.txt'.format(DATA_IN), header=None).dropna().astype(int)
    df.columns = ['job', 'geek']
    df['rating'] = 2

    df = iter_filter_old(df, T, T)
    train_frame, test_frame, neg_frame = train_test_split(df)
